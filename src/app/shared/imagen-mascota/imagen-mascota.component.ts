...
import { MatDialogRef, MAT_DIALOG_DATA, MatSnackBar, MatSnackBarVerticalPosition } from '@angular/material';
...
export class ImagenMascotaComponent {
  ...
  private onSubmit(event: Event) {
    ...
    if (this.uploader.files.length > 0) {
      ...
      this.archService.guardarImagen(formData)
        .subscribe(
          (resul: Array<string>) => {
            this.mascotaSuccess(resul, this.mascota, `Se ha guardado correctamente la imagen para ${this.mascota.nombre}`)
          },
          () => this.mascotaError(this.mascota, `No se ha podido guardar la imagen de la mascota ${this.mascota.nombre}`)
        );
    }
  }

  private mascotaSuccess(resul: Array<string>, mascota: IMascota, strSuccess: string) {
